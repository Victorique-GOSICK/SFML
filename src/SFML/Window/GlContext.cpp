...
void GlContext::initialize(const ContextSettings& requestedSettings)
{
    ...
    else
    {
        ...
        if (version && (std::strlen(version) >= 3))
        {
            if (!std::strncmp(version, "OpenGL ES-CL", 12))
            {
            ...
            else if (std::isdigit(version[0]) && std::isdigit(version[2]))
            {
